    ...
    public void run() {
        try {
            ...
            if (con.getResponseCode() == HttpURLConnection.HTTP_OK) {
                if (con.getContentLengthLong() != f.length()) {
                    byte[] fh = GitUtils.getGitHash(new FileInputStream(f), f.length(), GitUtils.GitType.BLOB);
                    if (!Arrays.equals(fh, IOUtils.hexToByteArray(con.getHeaderField("Etag").substring(1, con.getHeaderField("Etag").length() - 1)))) {
                        ...
                        System.out.println("Lang file " + this.locale + ".lang has been updated");
                    }
                }
